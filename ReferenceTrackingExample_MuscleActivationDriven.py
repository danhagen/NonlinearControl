...
while AnotherIteration3 == True:
	...
	temp_Tension = return_initial_U_tension_driven(0,np.array([Amp+Base,0]))
	temp_Vm = return_initial_U_muscle_velocity_driven(0,np.array([Amp+Base,0,\
																temp_Tension[0],temp_Tension[1]]))
	X3[:,0] = [Amp+Base, 0, temp_Tension[0], temp_Tension[1], 0.8*lo1, 1.2*lo2, 0, 0]
	...
	try:
		...
		for i in range(len(Time3)-1):
			U3[:,i+1] = return_U_muscle_activation_driven(i,Time3,X3[:,i],U3[:,i],Noise = NoiseArray3[:,i])
		...
		if AttemptNumber3 == 25: AnotherIteration3 = False
