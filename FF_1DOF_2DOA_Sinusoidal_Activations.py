from pendulum_eqns.sim_eqns_ActFF_sinusoidal_activations import *
...
if len(InitialTensions) != 0:
    figs = plot_N_sim_FF_sinus_act(Time,TotalX,TotalU,Return=True)
    ...
    save_figures("output_figures/feedforward_sinusoidal_activations/","1DOF_2DOA_v1.0",SaveAsPDF=True)
    plt.close('all')
    FormatedSaveData = {
            "States" : TotalX,
            "Input" : TotalU,
    ...
    pickle.dump(
        FormatedSaveData,
        open(
            "output_figures/feedforward_sinusoidal_activations/output.pkl",
            "wb"
            )
        )
else:
